...
import { Search, Bell, Menu, ChevronDown, User, LogOut, ChevronRight } from "lucide-react";
import {
  Avatar,
  Dropdown,
  DropdownItem,
...
export function Topbar({ onMenuClick }) {
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      setNotifications((res.tasks || []).slice(0, 5));
    } catch (err) {
    ...
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-line bg-canvas/80 px-4 backdrop-blur md:px-8">
      ...
      <div className="relative flex-1 max-w-md">
        ...
        {searchResults.length > 0 && (
          <div className="absolute top-12 left-0 right-0 max-h-64 overflow-y-auto rounded-xl border border-line bg-canvas shadow-lg z-50">
